            ...
            if (buttonType.getButtonData() == ButtonData.OK_DONE) {
                try {
                    if(category.getValue().getClass() == IncomeCategory.class) {
                        return Transaction.builder().description(description.getText()).date(date.getValue()).type(type.getValue()).amount(Float.parseFloat(amount.getText())).incomeCategory((IncomeCategory)category.getValue()).build();
                    } else {
                        return Transaction.builder().description(description.getText()).date(date.getValue()).type(type.getValue()).amount(Float.parseFloat(amount.getText())).expenseCategory((ExpenseCategory)category.getValue()).build();
                    }
